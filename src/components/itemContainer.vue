        ...
        <div class="item_list_container" v-if="itemDetail.length > 0">
          ...
          <ul>
            <li v-for="(item, index) in itemDetail[itemNum-1].topic_answer"
              ...
              <span class="option_style" v-bind:class="{'has_choosed':chosenNum==index}">{{chooseType(index)}}</span>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        itemId: null, //题目ID
        chosenNum: null, //选中答案索引
        chosenId: null //选中答案id
      }
    },
    ...
    methods: {
      ...
      //点击下一题
      nextItem() {
        if (this.chosenNum !== null) {
          this.chosenNum = null;
          //保存答案, 题目索引加一，跳到下一题
          this.addNum(this.chosenId)
        } else {
          alert('您还没有选择答案哦')
        }
      },
